...
public class Particle {
  ...
  public Particle(int x, int y, double vx, double vy, int diam, int mass, Color c) {
    this.x = x;
    this.y = y;
    v = new Vector(vx, vy);
    this.diam = diam;
  ...
  /**
   * Updates two colliding particles from their state immediately prior to collision to their state
   * immediately after. In particular, the velocities of the particles are updated.
   */
  public static void collide(Particle p1, Particle p2) {
    ...
    double newMagV1Normal =
        (magV1Normal * (p1.mass - p2.mass) + 2 * p2.mass * magV2Normal) / (p1.mass + p2.mass);
    double newMagV2Normal =
        (magV2Normal * (p2.mass - p1.mass) + 2 * p1.mass * magV1Normal) / (p1.mass + p2.mass);
  ...
  /**
   * Updates Particle p from its state immediately before colliding with Wall w to its state
   * immediately post the collision.
   */
